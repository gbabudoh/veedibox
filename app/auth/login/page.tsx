...
import { Film, Mail, Lock, ArrowRight, Loader2, AlertCircle, Sparkles } from "lucide-react";
        ...
        >
            ...
            <motion.div whileHover={{ scale: 1.05 }} className="mb-6">
              <Link href="/" className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col">
                  <span className="text-xl font-black tracking-tighter uppercase text-glow leading-none pt-0.5">Veedibox</span>
                  <span className="text-[8px] font-bold tracking-[0.25em] uppercase opacity-30 ml-0.5">Productions</span>
                </div>
              </Link>
            </motion.div>
            <h1 className="text-2xl font-black tracking-tight mb-1.5 bg-clip-text text-transparent bg-gradient-to-b from-white to-white/50">Welcome back</h1>
            <p className="text-foreground/30 text-xs">Sign in to your account</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.div variants={itemVariants} className="space-y-1.5">
              <label className="text-[11px] font-semibold text-white/40 ml-1">Email</label>
              <div className="relative">
                <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'email' ? 'text-primary' : 'text-white/20'}`}>
                  <Mail size={16} />
                </div>
                <input 
                  type="email"
            ...
            <motion.div variants={itemVariants} className="space-y-1.5">
              <div className="flex justify-between items-center ml-1">
                ...
                <button type="button" className="text-[10px] font-semibold text-primary/50 hover:text-primary transition-colors">Forgot?</button>
              </div>
              <div className="relative">
                <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'password' ? 'text-primary' : 'text-white/20'}`}>
                  <Lock size={16} />
                </div>
                <input 
                  type="password"
              ...
              >
                {isLoading ? (
                  ...
                  <div className="flex items-center justify-center gap-2 uppercase font-bold">
                    Sign In <ArrowRight size={16} className="group-hover/btn:translate-x-0.5 transition-transform" />
                  </div>
                )}
              </CinemaButton>
            </motion.div>
          ...
          <motion.div variants={itemVariants} className="mt-6 text-center">
            <p className="text-white/25 text-xs mb-2">Don&apos;t have an account?</p>
            <Link 
              href="/auth/register" 
